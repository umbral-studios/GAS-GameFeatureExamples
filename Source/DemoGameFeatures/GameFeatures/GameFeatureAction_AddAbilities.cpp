...
#include "DemoGameFeatures/AbilitySystem/DemoGameFeaturesAbilitySystemComponent.h"
#include "DemoGameFeatures/AbilitySystem/DemoGameFeaturesAbilityAttributeSet.h"
#include "DemoGameFeatures/AbilitySystem/AbilityInputBindingComponent.h"

#define LOCTEXT_NAMESPACE "DemoGameFeaturesFeatures"
...
void UGameFeatureAction_AddAbilities::AddAdditionalAssetBundleData(FAssetBundleData& AssetBundleData)
{
	if (UAssetManager::IsValid())
	{
		...
		{
			for (const FDemoGameFeaturesAbilityMapping& Ability : Entry.GrantedAbilities)
			{
			...
			for (const FDemoGameFeaturesAttributesMapping& Attributes : Entry.GrantedAttributes)
			{
...
{
	UE_LOG(LogDemoGameFeatures, Error, TEXT("Added ability"), *Actor->GetPathName());
	if (UAbilitySystemComponent* AbilitySystemComponent = FindOrAddComponentForActor<UAbilitySystemComponent>(Actor, AbilitiesEntry))
	{
		...
		for (const FDemoGameFeaturesAbilityMapping& Ability : AbilitiesEntry.GrantedAbilities)
		{
			if (!Ability.AbilityType.IsNull())
			{
				...
				if (!Ability.InputAction.IsNull())
				{
					...
					{
						 UE_LOG(LogDemoGameFeatures, Error, TEXT("Failed to find/add an ability input binding component to '%s' -- are you sure it's a pawn class?"), *Actor->GetPathName());
					}
				}
		...
		for (const FDemoGameFeaturesAttributesMapping& Attributes : AbilitiesEntry.GrantedAttributes)
		{
			if (!Attributes.AttributeSetType.IsNull())
			{
				TSubclassOf<UDemoGameFeaturesAbilityAttributeSet> SetType = Attributes.AttributeSetType.LoadSynchronous();
				if (SetType)
				{
	...
	{
		UE_LOG(LogDemoGameFeatures, Error, TEXT("Failed to find/add an ability component to '%s'. Abilities will not be granted."), *Actor->GetPathName());
	}
}
